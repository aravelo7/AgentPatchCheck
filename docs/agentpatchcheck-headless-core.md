...
agentpatchcheck run --task-spec .\task-spec.json

# Repository development convenience wrapper
npm.cmd run agentpatchcheck:run -- --task-spec .\task-spec.json
```
...
On each iteration the model may request structured read, bounded directory-search, exact-text patch, exclusive file-create, or finish/fail operations. `search-text-recursive` is a separate read-only tool with fixed depth, file-count, byte, and match limits; it skips state, dependency, build, common credential, binary, and symbolic-link paths. The Tool Broker validates every request before execution and only permits regular, non-symlink files beneath the managed worktree; absolute paths, `..`, `.git`, `.agentpatchcheck`, and unregistered tools are rejected. The Harness—not the model—collects the patch, runs verification and Hidden Oracle, writes Evidence, evaluates risk, and controls apply.

The Provider Registry supports the official OpenAI endpoint and explicitly configured OpenAI-compatible endpoints through either `responses` or `chat-completions` protocol. An OpenAI-compatible TaskSpec must use `provider: "openai-compatible"`, an HTTPS `baseUrl` (local HTTP only for tests), a supported protocol, and one of the fixed credential references. `thinkingMode: "disabled"` is available only for OpenAI-compatible Chat Completions providers that require non-thinking mode for forced function calling; otherwise it defaults to `"default"`. The current resolver maps `openai-primary`, `openai-secondary`, `deepseek-primary`, `provider-a-primary`, and `provider-b-primary` to dedicated environment variables; it never accepts an environment variable name from a TaskSpec. There is no credential rotation, fallback, or arbitrary provider routing.
...
```powershell
npm.cmd run agentpatchcheck:assess -- --evidence <path-to-target-repository>\.agentpatchcheck\evidence\<runId>.json
```

After assessment, preview cleanup of that run's managed worktree. Add `--apply` only after reviewing the printed target:

```powershell
npm.cmd run agentpatchcheck:cleanup -- --evidence <path-to-target-repository>\.agentpatchcheck\evidence\<runId>.json
